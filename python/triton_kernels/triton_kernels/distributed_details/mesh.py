...
class _TorchMesh:
    ...
    def process_group(self) -> dist.ProcessGroup:
        if self._process_group is None:
            ...
            assert self._process_group is not None, (
                "process_group called before comms initialization"
            )
    ...
    def get_global_rank(self, group_rank: int) -> int:
        return (
            ...
            else dist.get_global_rank(self.process_group, group_rank)
        )
    ...
    def get_group_rank(self, global_rank: int) -> int:
        return (
            global_rank
            if self.process_group is None
            else dist.get_group_rank(self.process_group, global_rank)
        )
...
class SymmetricMemoryPool:
    ...
    def make_empty(
        ...
        if region_offset % elem_size != 0:
            raise ValueError(
                f"Region offset {region_offset} not aligned to element size {elem_size}"
            )
    ...
    def _initialize(
        ...
        if self.mesh.world_size > 1:
            ...
            self.bufs = tuple(
                ...
                for r in range(self.mesh.world_size)
            )
    ...
    def initialize_matmul(
        ...
        BLOCK_M = 32
        n_bytes_topk = (
            n_tokens_global * n_expts_act * 4
        )  # topk logits (float32): pessimistic estimate
        ...
        n_bytes_topk += (
            num_blocks_m * BLOCK_M * num_blocks_n * BLOCK_N // 32 * 4
        )  # expt bitmatrix (int32)
        elem_size = torch.empty((), dtype=dtype).element_size()
        n_bytes_dp_to_ep = n_tokens_global * n_expts_act * d_input * elem_size
        n_bytes_ep_to_dp = (
            (n_tokens_global // self.mesh.world_size) * n_expts_act * d_model * elem_size
        )
